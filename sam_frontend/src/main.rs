...
pub fn Hero() -> Element {
    ...
    let ff = move |_| async move {
        ...
        match Request::post("http://127.0.0.1:3000/users/login")
    ...
    let ttt = move |_| async move {
        match Request::get("http://127.0.0.1:3000/users/test")
            .credentials(RequestCredentials::Include)
            .send()
            .await
        {
