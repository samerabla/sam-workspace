...
use sam_util::{fetch_data, post_json};
...
pub fn AddCategory() -> Element {
    ...
    let names: Signal<HashMap<String, CategoryName>> = use_signal(HashMap::new);
    ...
    let send_request = move |_| {
        ...
        async move {
            ...
            match post_json(&url, &category_with_names).await {
                Ok(res) => {
                    spinner_state.set(PopupState::Close);
                    let user_res: UserResponse = res.json().await.unwrap();
                    if res.ok() {
                        msg.set(MsgConfig::with_success(user_res.message()));
                        reset();
                    } else {
    ...
    rsx! {
        div { class: "categories",
            ...
            button { class: "btn btn-primary", onclick: send_request, "Add Category" }
            {
